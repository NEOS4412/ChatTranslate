...
def run(book: Path, titles: bool, sups: bool) -> None:
    ...
    if not files:
        print("no translated pages found"); return
    # 合并再清（保留页分隔符），便于校对
    ...
    for f in files:
        ...
        # proofread/full_zh.md 中图片是 images/imgs/xxx.jpg
        # 实际位于 ocr/images/imgs/xxx.jpg
        t = t.replace('src="images/imgs/', 'src="images/imgs/')  # 链接到根目录
        parts.append(t)
    big = "\n\n".join(parts)
    merged.write_text(big, encoding="utf-8")
    changed = clean_file(merged, titles, sups)
    print(f"[clean] merged {len(files)} pages -> {merged} ({'changed' if changed else 'no change'})")
