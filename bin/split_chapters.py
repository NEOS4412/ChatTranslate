...
def run(book: Path, front: int, protect: bool = False) -> None:
    ...
    chap_dir = book / "chapters"
    chap_dir.mkdir(exist_ok=True)
    if protect:
        # 把要保护的章节复制到临时目录
        import shutil, tempfile
        backup = Path(tempfile.mkdtemp(prefix="chap_protect_"))
        for f in chap_dir.glob("*.md"):
            shutil.copy2(f, backup / f.name)
        print(f"[protect] backed up {len(list(chap_dir.glob('*.md')))} files to {backup}")
    for f in chap_dir.glob("*.md"): f.unlink()
    ...
    if protect:
        ...
        import shutil
        restored = 0
        for f in backup.glob("*.md"):
            shutil.copy2(f, chap_dir / f.name)
            restored += 1
        print(f"[protect] restored {restored} manual chapters")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--front", type=int, default=7, help="前 N 个章节视为前置（不显示）")
    ap.add_argument("--protect", action="store_true", help="保留已存在的章节文件（手动修改优先）")
    args = ap.parse_args()
