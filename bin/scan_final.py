...
def check_untranslated(text: str, src_lang: str = "fr") -> list[dict]:
    ...
    for para in paragraphs:
        ...
        if cn == 0 and len(stripped) > 60:
            line = text[:text.find(stripped)].count('\n') + 1
            issues.append({
                "line": line,
                "type": "untranslated",
...
def fix_issues(filepath: Path, src_lang: str = "fr", key: str | None = None, fix_merge: bool = False) -> dict:
    ...
    # 3. 清理其他 LaTeX 残留
    new_text, n = re.subn(r'\$[^$]{1,50}\$', '', new_text)
    if n > 0:
...
if __name__ == "__main__":
    ...
    ap.add_argument("book", type=Path, help="书籍目录")
    ...
    ap.add_argument("--fix-translate", action="store_true", help="自动翻译未翻译段落")
    ap.add_argument("--fix-merge", action="store_true", help="合并句间断行")
    args = ap.parse_args()
    ...
    if args.fix or args.fix_translate or args.fix_merge:
        ...
        scan_book(args.book, args.src_lang, args.workers)
